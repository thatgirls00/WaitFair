...
class PreRegisterControllerTest {
	...
	class RegisterPreRegister {
		...
		void register_Success() throws Exception {
			...
			mockMvc.perform(post("/api/v1/events/{eventId}/pre-registers", testEvent.getId())
					.param("userId", String.valueOf(testUser.user().getId()))
		...
		@Test
		@DisplayName("이름이 일치하지 않으면 400 에러 (본인 인증 실패)")
		void register_Fail_InvalidName() throws Exception {
			// given: 잘못된 이름
			PreRegisterCreateRequest request = PreRegisterRequestFactory.fakePreRegisterRequest(
				"wrongNickname",  // 실제 닉네임과 다름
				testUser.rawPassword(),
				testUser.user().getBirthDate()
			);
		...
		@Test
		@DisplayName("비밀번호가 일치하지 않으면 400 에러 (본인 인증 실패)")
		void register_Fail_InvalidPassword() throws Exception {
			// given: 잘못된 비밀번호
			PreRegisterCreateRequest request = PreRegisterRequestFactory.fakePreRegisterRequest(
				testUser.user().getNickname(),
				"wrongpassword",  // 실제 비밀번호와 다름
				testUser.user().getBirthDate()
			);
			...
			mockMvc.perform(post("/api/v1/events/{eventId}/pre-registers", testEvent.getId())
				...
				.andDo(print())
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.message").value(PreRegisterErrorCode.INVALID_PASSWORD.getMessage()));
		}
		...
		@Test
		@DisplayName("이름이 비어있으면 400 에러 (Validation)")
		...
		@Test
		@DisplayName("비밀번호가 6자 미만이면 400 에러 (Validation)")
	...
	class ScenarioAndEdgeCaseTests {
		...
		@Test
		@DisplayName("존재하지 않는 사용자로 사전등록 시도 시 404 에러")
		void register_Fail_NonExistentUser() throws Exception {
			// given
			...
			mockMvc.perform(post("/api/v1/events/{eventId}/pre-registers", testEvent.getId())
				...
				.andDo(print())
				.andExpect(status().isNotFound())
				.andExpect(jsonPath("$.message").value(CommonErrorCode.NOT_FOUND_USER.getMessage()));
		}
